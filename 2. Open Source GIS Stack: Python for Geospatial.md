...
Now if I go to QGIS then DB Manager, I can find the newly added table, named newpoints, under the PostGIS -->  tompkins --> Public. I can drag and drop it in the lft panel under Layers then I can go to the web --> QuickMapServices --> OSM --> OSM Standard to have the background map on top of my points, which is pretty cool. So now we have a workable GIS database.
