...
The cool thing is that we can use Python to talk to PostGIS and also Excel. Here is a code snippet which first reads addresses from a csv file then geocode these addresses to get their longitude and latitude, then writes them in a table in PostGIS:
